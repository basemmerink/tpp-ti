...
class TwitchModule extends OAuthModule
{
    ...
    onTokensLoaded(hasAccessToken: boolean): void
    {
        ...
        else
        {
            ...
            open(`https://id.twitch.tv/oauth2/authorize?client_id=${process.env.TWITCH_CLIENT_ID}&redirect_uri=https://${process.env.EXTERNAL_DOMAIN_OR_IP}/twitch&scope=${scope.join('%20')}&force_verify=true&response_type=code`);
        }
    }
    ...
    public createSubscription(type: string, condition: any): void
    {
        this.doPost('/eventsub/subscriptions', undefined, undefined, {
            ...
            transport: {
                method: 'webhook',
                callback: `${process.env.EXTERNAL_DOMAIN_OR_IP}/eventsub`,
